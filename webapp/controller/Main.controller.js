sap.ui.define([
    ...
    function (BaseController, JSONModel) {
        ...
        return BaseController.extend("lessons.controller.Main", {
            onInit: function () {
                var oViewModel = new JSONModel({
                    busy: false,
                    delay: 0,
                    oStandard: "lesson_no,lesson_date,init_hour,end_hour,created_by,created_at",
                    oSmartTableView: "",
            ...
            onPressLessonDetail: function (oEvent) {
            ...
            onCreateLesson: function () {
                sessionStorage.setItem("goToLaunchpad", "");
                this.onRemoveSelection();
                this.onNavigation("", "createLesson", "");
            },

            onDeleteLesson: function () {
                var that = this,
                    oModel = this.getModel(),
                    oTable = sap.ui.getCore().byId("lessonsTable"),
                    sPath = oTable.getTable().getSelectedItem().getBindingContextPath(),
                    oMessage = {
                        oText: "lessonDeletedText",
                        oTitle: "lessonDeletedTitle"
                    };

                if (sPath) {
                    new sap.m.MessageBox.warning(this.getResourceBundle().getText("deleteLessonText"), {
                        title: this.getResourceBundle().getText("deleteLessonHeader"),
                        actions: [sap.m.MessageBox.Action.OK, sap.m.MessageBox.Action.CANCEL],
                        emphasizedAction: sap.m.MessageBox.Action.OK,
                        onClose: function (oAction) {
                            if (oAction === sap.m.MessageBox.Action.OK) {
                                sap.ui.getCore().byId("deleteLesson").setEnabled(false);
            ...
            onSelectionChange: function () {
                sap.ui.getCore().byId("deleteLesson").setEnabled(true);
            },

            onRemoveSelection: function () {
                var oTable = sap.ui.getCore().byId("lessonsTable").getTable();
                oTable.removeSelections();
                sap.ui.getCore().byId("deleteLesson").setEnabled(false);
            },
            ...
            onBuildSmartTable: function () {
                var oOldSmartTable = sap.ui.getCore().byId("lessonsTable");
                if (oOldSmartTable) {
                    oOldSmartTable.destroy();
                }

                var oDestroyedSmartTable = sap.ui.getCore().byId("lessonsTable");
                if (!oDestroyedSmartTable) {
                    ...
                    var oCreateButton = new sap.m.Button({
                        id: "createLesson",
                        text: "{i18n>createLesson}",
                        press: this.onCreateLesson.bind(this)
                    });

                    var oDeleteButton = new sap.m.Button({
                        id: "deleteLesson",
                        text: "{i18n>deleteLesson}",
                        enabled: false,
                        press: this.onDeleteLesson.bind(this)
                    });
                    ...
                    var oSmartTable = new sap.ui.comp.smarttable.SmartTable({
                        id: "lessonsTable",
                        entitySet: "xTQAxLESSONS_DD",
                        smartFilterId: "smartFilterBarGroups",
                        tableType: "ResponsiveTable",
                        header: "{i18n>lessonsTableTitle}",
                        showRowCount: true,
                                ...
                                if (oItems.length > 0) {
                                    ...
                                    oItems.forEach(oItem => {
                                        if (oItem instanceof sap.m.ColumnListItem) {
                                            ...
                                            oItem.attachPress(this.onPressLessonDetail.bind(this));
                                        }
                                    });
                                }
